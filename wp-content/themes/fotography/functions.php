...
function fotography_custom_stylesheet(){
	...
	<style type="text/css">
		<?php 
			...
			$ft_rgba_bg_color = fotography_hex2rgba($fg_theme_color,0.65);
...
/******************************* Pagination ***************************************/
function fotography_pagination($pages = '', $range = 2){
     $showitems = ($range * 2)+1;  
      global $paged;
     ...
     if(1 != $pages)
     {
         echo "<div class=\"pagination\"><span>Page " . esc_html($paged) . esc_html( ' of ', 'fotography' ) . esc_html($pages) . "</span>";
...
function fotography_hex2rgba($color, $opacity = false) { 
	 $default = 'rgb(0,0,0)'; 
